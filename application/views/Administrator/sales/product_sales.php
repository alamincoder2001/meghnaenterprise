...
<div id="sales" class="row">
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
		<div class="row">
			...
			<div class="form-group">
				<label class="col-sm-1 control-label no-padding-right"> <?php if($para=='product'){
					echo 'Sales ';
				}else if($para=='quotation'){
					echo 'Quotation';
				}else{
					echo 'Order';
				} ?> By </label>
	...
	<div class="col-xs-9 col-md-9 col-lg-9">
		<div class="widget-box">
			<div class="widget-header">
				<h4 class="widget-title"><?php if($para=='product'){
					echo 'Sales ';
				}else if($para=='quotation'){
					echo 'Quotation';
				}else{
					...
					<div class="row">
						<div class="col-sm-5">
							<div class="form-group clearfix" style="margin-bottom: 8px;">
								<label class="col-sm-4 control-label no-padding-right"> <?php if($para=='product'){
					echo 'Sales ';
				}else if($para=='quotation'){
					echo 'Quotation';
				}else{
		...
		<div class="widget-box">
			...
			<div class="widget-body">
				<div class="widget-main">
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-sm-12 control-label">Delivery Site Address</label> 
												<div class="col-sm-11">
													<textarea v-model="sales.delivery_address">
									...
									<tr>
										<td>
											<div class="form-group"> 
												<label class="col-sm-12 control-label"><?php if($para=='product'){
					echo 'Sales ';
				}else if($para=='quotation'){
					echo 'Quotation';
				}else{
					...
					<input style="margin-left: 11px;margin-bottom: 15px;" type="radio" name="sales_type" value="deliver" v-model="sales.status"> Deliver
						

						<?php if($para=='quotation'){?>
							<input type="radio" name="sales_type" value="undeliver" v-model="sales.status"> Undeliver 
				<?php }else{?> 
					<input type="radio" name="sales_type" value="undeliver" v-model="sales.status"> Undeliver 
				<?php } ?>
												<!-- <v-select  class="col-sm-12"id="status" v-bind:options="statuses" label="status" v-model="selectedStatus" ></v-select> -->
											</div>
										</td>
									</tr>
	...
	new Vue({
		...
		data(){
			return {
				...
				discountPercent: 0,
				cart: [],
				statuses:[{status:'deliver'},{status:'undeliver'}],
				employees: [],
		...
		methods:{
			...
			async customerOnChange(){
				...
				if(this.selectedCustomer.Customer_SlNo=='C01'){
					this.sales.paid = this.sales.due
					this.sales.due = 0
				}
			},
			...
			async saveSales(){
				...
				// this.sales.status = this.selectedStatus == null || this.selectedStatus.status;
			...
			getSales(){
				axios.post('/get_sales', {salesId: this.sales.salesId}).then(res=>{
					...
					this.sales.delivery_address = sales.delivery_address
					this.sales.status = sales.sales_type;
