	...
	<div class="row" style="border-bottom: 1px solid #ccc;">
		<div class="col-md-12">
			<form class="form-inline" v-on:submit.prevent="getProfitLoss">
				...
				<div class="form-group" style="margin-right: 15px;">
					<label>User &nbsp;</label>
					<v-select v-bind:options="users" v-model="selectedUser" label="User_Name" placeholder="Select user"></v-select>
				</div>
			...
			<div class="table-responsive" id="reportTable">
				<table id="table1">
					...
					<tfoot style="display:none;font-weight:bold;background-color:#e9dcdc;" v-bind:style="{display: reportData.length > 0 ? '' : 'none'}">
						...
						<tr>
							<td colspan="4" style="text-align:right;">Cancel profit Value (-)</td>
							<td colspan="2"></td>
							<td style="text-align:right;">{{ cancleProfit | decimal }}</td>
						</tr>
		...
		methods: {
			...
			async getProfitLoss(){
				...
				if(this.selectedUser != null){
					this.filter.user = this.selectedUser.User_Name;
				} else {
				...
				this.cancleProfit = await axios.post('/get_cancle_profit',this.filter).then(res => {
					return res.data;
				});
