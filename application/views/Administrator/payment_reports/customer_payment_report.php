			...
			<table class="table table-bordered">
				...
				<tbody>
					...
					<tr v-if="payments.length > 0">
						...
						<th style="text-align:right">{{payments.reduce((acc, pre) => {return acc + +parseFloat(pre.bill)},0).toFixed(2)}}</th>
						<th style="text-align:right">{{payments.reduce((acc, pre) => {return acc + +parseFloat(pre.paid)},0).toFixed(2)}}</th>
						<th style="text-align:right">{{payments.reduce((acc, pre) => {return acc + +parseFloat(pre.discount)},0).toFixed(2)}}</th>
						<th style="text-align:right">{{payments.reduce((acc, pre) => {return acc + +parseFloat(pre.due)},0).toFixed(2)}}</th>
						<th style="text-align:right">{{payments.reduce((acc, pre) => {return acc + +parseFloat(pre.returned)},0).toFixed(2)}}</th>
						<th style="text-align:right">{{payments.reduce((acc, pre) => {return acc + +parseFloat(pre.paid_out)},0).toFixed(2)}}</th>
						<th></th>
					</tr>
				</tbody>
		...
		methods:{
			...
			getReport(){
				...
				axios.post('/get_customer_ledger', data).then(res => {
					this.payments = res.data.payments.filter(pay => {
						return pay.date >= this.dateFrom && pay.date <= this.dateTo
					});
