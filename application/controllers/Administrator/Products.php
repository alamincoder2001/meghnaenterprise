...
class Products extends CI_Controller {
    ...
    //                     and dm.Damage_brunchid = '$branchId' and dm.Status='a') as damaged_quantity,
    ...
    //         $inventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ?", [$key->Product_SlNo, $this->session->userdata('BRANCHid')])->num_rows();
    //         if($inventoryCount == 0){
    //             $inventory = array(
    //                 'product_id' => $key->Product_SlNo,
    //                 'purchase_quantity' => $key->purchased_quantity,
    //                 'purchase_return_quantity' => $key->purchase_returned_quantity,
    //                 'sales_quantity' => $key->sold_quantity,
    //                 'sales_return_quantity' => $key->sales_returned_quantity,
    //                 'damage_quantity' => $key->damaged_quantity,
    //                 'transfer_from_quantity' => $key->transferred_from_quantity,
    //                 'transfer_to_quantity' => $key->transferred_to_quantity,
    ...
    //         }
            ...
            //         where product_id = ? 
            //         and branch_id = ?
            //     ", [$key->purchased_quantity, $key->Product_SlNo, $this->session->userdata('BRANCHid')]);
            // }
            //$this->db->insert('tbl_currentinventory',$inventory);
    //     }
