...
class Sales extends CI_Controller {
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(dmd.DamageDetails_DamageQuantity), 0) 
                        ...
                        where dmd.Product_SlNo = p.Product_SlNo and dmd.status='a' 
            ...
            where p.status = 'a' and p.is_service = 'false' and p.Product_branchid=?  
        ",$branchId)->result();
    ...
    foreach($stock as $key=>$val){
        $count =     $this->db->query('select * from  tbl_currentinventory where product_id=?  and branch_id=?',[$val->Product_SlNo,$branchId])->num_rows();
        if($count>0){
            /// update
            $this->db->query('update tbl_currentinventory 
             ...
             transfer_from_quantity = transfer_from_quantity + ? ,
             transfer_to_quantity = transfer_to_quantity + ? 
             where product_id = ?  and
                  branch_id=?
              ',[$val->purchased_quantity,$val->purchase_returned_quantity,
              $val->sold_quantity,
              $val->sales_returned_quantity,
              $val->damaged_quantity,
              $val->transferred_from_quantity,
              $val->transferred_to_quantity,
              $val->Product_SlNo,
            ...
            //  insert
            $data = array(
                'product_id'=>$val->Product_SlNo,
                'purchase_quantity'=>$val->purchased_quantity,
                'purchase_return_quantity'=>$val->purchase_returned_quantity,
                'sales_quantity'=>$val->sold_quantity,
                'sales_return_quantity'=>$val->sales_returned_quantity,
                'damage_quantity'=>$val->damaged_quantity,
                'transfer_from_quantity'=>$val->transferred_from_quantity,
                'transfer_to_quantity'=>$val->transferred_to_quantity,
                'branch_id'=>$branchId
            );
            $this->db->insert('tbl_currentinventory',$data);
        }
    ...
    public function addSales(){
        ...
        try{
            ...
            $sales = array(
                ...
                'sales_type'=>$data->sales->status,
                'delivery_address'=>$data->sales->delivery_address
            );
    ...
    public function getSaleDetails(){
        ...
        if(isset($data->fetchType) && $data->fetchType=='p'){
            $clauses .= " and sm.sales_type='undeliver'";
        }
    ...
    public function getSales(){
        ...
        if(!isset($data->invoice)){
            ...
            if(isset($data->fetchType) && $data->fetchType=='a'){
                $clauses .= " and sm.sales_type='deliver'";
            }
            if(isset($data->fetchType) && $data->fetchType=='q'){
                $clauses .= " and sm.sales_type='quotation'";
            }
        }
        ...
        $sales = $this->db->query("
            select 
            sm.*,
            SaleMaster_InvoiceNo as invoice,
            concat(c.Customer_Name,' - ',c.Customer_Mobile) as Customer_Name,
            c.Customer_Code,
            c.Customer_Name,
            c.Customer_Mobile,
            c.Customer_Address,
    ...
    public function checkInv(){
        $data = json_decode($this->input->raw_input_stream); 
        $sm = $this->db->query("SELECT * FROM tbl_salesmaster WHERE SaleMaster_SlNo=? LIMIT 1",$data->salesId)->result();
        echo json_encode(['status'=>$sm[0]->Status]);
    }
    ...
    public function orderConfirm(){
        ...
        $this->db->where('SaleMaster_SlNo', $data->id);
        $this->db->update('tbl_salesmaster', $sales);
        
        $currentSaleDetails = $this->db->query("select * from tbl_saledetails where SaleMaster_IDNo = ?", $data->id)->result();
        
        foreach($currentSaleDetails as $cartProduct){
  
                           
            $checkOnCurrentStock = $this->db->query("SELECT * FROM tbl_currentinventory WHERE product_id=? and branch_id = ?",[$cartProduct->Product_IDNo,$this->session->userdata('BRANCHid')])->num_rows();
            if($checkOnCurrentStock>0){
            //update stock 
            $this->db->query("
                update tbl_currentinventory 
            ...
            ", [$cartProduct->SaleDetails_TotalQuantity, $cartProduct->Product_IDNo, $this->session->userdata('BRANCHid')]);
                ...
                $data = array(
                    'product_id'=>$cartProduct->Product_IDNo,
                    'sales_quantity'=>$cartProduct->SaleDetails_TotalQuantity,
                    'branch_id'=>$this->session->userdata('BRANCHid')
                );
                $this->db->insert('tbl_currentinventory',$data);
    ...
    public function updateSales(){
        ...
        try{
            ...
            $currentSale = $this->db->query("select * from tbl_salesmaster sm where sm.SaleMaster_SlNo = ?", $salesId)->row();
            ...
            if($currentSale->sales_type == 'deliver') {
            ...
            foreach($data->cart as $cartProduct){
                ...
                if($data->sales->status=='deliver'){
                    ...
                    if($checkOnCurrentStock>0){
                        ...
                        $this->db->insert('tbl_currentinventory', $inventoryData);
                    }
                }
            }
    ...
    public function addSalesReturn(){
        ...
        try{
            $data = json_decode($this->input->raw_input_stream);
            $salesReturn = array(
                'SaleMaster_InvoiceNo' => $data->invoice->invoice,
            ...
            foreach($data->cart as $product){
             ...
             $totalCalcleProfit =  $totalCalcleProfit+(($product->SaleDetails_Rate-$product->Purchase_Rate)*$product->return_quantity);
                $returnDetails = array(
            ...
            $dataS = array(
                'cancle_profit'=>$totalCalcleProfit
            );

            $this->db->where('SaleReturn_SlNo',$salesReturnId);
            $this->db->update('tbl_salereturn',$dataS);
            ...
            if($customerInfo->Customer_Type == 'G') {
                $customerPayment = array(
                    ...
                    'CPayment_invoice' => $data->invoice->invoice,
    ...
    function quotation_record()  {
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = "Sales Record";  
        $data['content'] = $this->load->view('Administrator/sales/quotation_record', $data, TRUE);
        $this->load->view('Administrator/index', $data); 
    }
...
//    }
    ...
    public function getProfitLoss(){
        ...
        if($data->user != null || $data->user != ''){
            $customerClause .= " and sm.AddBy='$data->user'" ;
        }
    ...
    public function getCancleProfit(){
        $data = json_decode($this->input->raw_input_stream);
        $dateClause = "";
        if(($data->dateFrom != null && $data->dateFrom != '') && ($data->dateTo != null && $data->dateTo != '')){
            $dateClause = " and sr.SaleReturn_ReturnDate between '$data->dateFrom' and '$data->dateTo'";
        } 
        $cancle_profits =  $this->db->query("select  sr.cancle_profit,sr.SaleMaster_InvoiceNo 
        from tbl_salereturn sr where sr.Status='a' and sr.SaleReturn_brunchId=? $dateClause ",
        [$this->session->userdata('BRANCHid')])->result();

        $amount = 0;

        foreach($cancle_profits as $key=>$val){
           $exits = $this->db->query("select * from tbl_salesmaster  where employee_id=? and SaleMaster_InvoiceNo=?",[$data->employee,$val->SaleMaster_InvoiceNo])->num_rows();
           if($exits > 0){
             $amount = $amount+$val->cancle_profit;
           }
        }
