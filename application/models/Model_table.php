    ...
    public function customerDue($clauses = "") {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(cp.CPayment_amount), 0.00) 
                ...
                (select ifnull(sum(cp.CPayment_amount), 0.00) 
                from tbl_customer_payment cp 
                where cp.CPayment_customerID = c.Customer_SlNo and cp.CPayment_TransactionType = 'DC'
            ...
            (select (billAmount + paidOutAmount) - (paidAmount + returnedAmount+discount)) as dueAmount
